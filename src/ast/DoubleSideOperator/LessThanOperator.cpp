...
LessThanOperator::LessThanOperator(NodePtr Left, NodePtr Right)
{
...
// Visualizing
void LessThanOperator::generateParser(std::ostream &dst, std::string indent) const
{
  dst << indent << "LessThan [" << std::endl;
...
// Context
void LessThanOperator::generateContext(ContextPtr frameContext, int parameterOffset, bool isGlobal  ) {
...
// MIPS
void LessThanOperator::generateMIPS(std::string indent, int destReg, ContextPtr frameContext) const
{
  switch(type){
    case _float:{
    ...
    default:{
      ...
      std::cout << "slt" << indent << "$"<<destReg<< ",$"<<leftReg<<",$"<<rightReg<<std::endl;
...
// Functions
bool LessThanOperator::isAssignmentExp() const
{
  return false;
}
void LessThanOperator::countFunctionParameter(int* functionCallParameterOffset) const
{
...
int LessThanOperator::getValue() const{
  return (branches[0]->getValue() < branches[1]->getValue());
}

enum Specifier LessThanOperator::returnType() const{
  return type;
}

void LessThanOperator::generateMIPSDouble(std::string indent, int destReg, ContextPtr frameContext) const{ 
  std::string number_label = frameContext->getFloatDoubleLabel(1.0, type);
  std::string label1 = makeName("L");
  std::string label2 = makeName("L");
  int leftReg = frameContext->alloFloatCalReg();
