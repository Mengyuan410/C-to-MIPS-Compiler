...
NotEqualToOperator::NotEqualToOperator(NodePtr Left, NodePtr Right)
{
  branches.push_back(Left);
  branches.push_back(Right);
}

NotEqualToOperator::~NotEqualToOperator()
{
  delete branches[0];
  delete branches[1];
}
...
// Visualizing
void NotEqualToOperator::generateParser(std::ostream &dst, std::string indent) const
{
  dst << indent << "NotEqualTo [" << std::endl;
...
// Context
void NotEqualToOperator::generateContext(ContextPtr frameContext, int parameterOffset, bool isGlobal  ) {
...
// MIPS
void NotEqualToOperator::generateMIPS(std::string indent, int destReg, ContextPtr frameContext) const
{
  switch(type){
    case _float:{
    ...
    default:{
      ...
      std::cout << "sltu" << indent << "$"<<destReg<< ",$0" <<",$"<<destReg<<std::endl;
...
bool NotEqualToOperator::isAssignmentExp() const
{
  return false;
}

void NotEqualToOperator::countFunctionParameter(int* functionCallParameterOffset) const
{
...
int NotEqualToOperator::getValue() const{
  return (branches[0]->getValue() != branches[1]->getValue());
}

enum Specifier NotEqualToOperator::returnType() const{
  return type;
}


void NotEqualToOperator::generateMIPSDouble(std::string indent, int destReg, ContextPtr frameContext) const{ 
  std::string number_label = frameContext->getFloatDoubleLabel(1.0, type);
  std::string label1 = makeName("L");
  std::string label2 = makeName("L");
  int leftReg = frameContext->alloFloatCalReg();
