...
EqualToOperator::EqualToOperator(NodePtr Left, NodePtr Right)
{
...
// Visualizing
void EqualToOperator::generateParser(std::ostream &dst, std::string indent) const
{
  dst << indent << "EqualTo [" << std::endl;
...
// Context
void EqualToOperator::generateContext(ContextPtr frameContext, int parameterOffset, bool isGlobal  ) {
...
// MIPS
void EqualToOperator::generateMIPS(std::string indent, int destReg, ContextPtr frameContext) const
{
  switch(type){
    case _float:{
      std::string number_label = frameContext->getFloatDoubleLabel(1.0, _float);
    ...
    case _double:{
      std::string number_label = frameContext->getFloatDoubleLabel(1.0, _double);
      std::string label1 = makeName("L");
      std::string label2 = makeName("L");
      int leftReg = frameContext->alloFloatCalReg();
      int rightReg = frameContext->alloFloatCalReg();
    ...
    default:{
      ...
      std::cout << "sltu" << indent << "$" << destReg << ",1" << std::endl;
...
bool EqualToOperator::isAssignmentExp() const
{
  return false;
}
void EqualToOperator::countFunctionParameter(int* functionCallParameterOffset) const
{
...
int EqualToOperator::getValue() const{
  return (branches[0]->getValue() == branches[1]->getValue());
}

enum Specifier EqualToOperator::returnType() const{
  return type;
}

void EqualToOperator::generateMIPSDouble(std::string indent, int destReg, ContextPtr frameContext) const
{
  std::string number_label = frameContext->getFloatDoubleLabel(1.0, _double);
  std::string label1 = makeName("L");
  std::string label2 = makeName("L");
