...
LessThanEqualToOperator::LessThanEqualToOperator(NodePtr Left, NodePtr Right)
{
  branches.push_back(Left);
  branches.push_back(Right);
}

LessThanEqualToOperator::~LessThanEqualToOperator()
{
  delete branches[0];
  delete branches[1];
}
...
// Visualizing
void LessThanEqualToOperator::generateParser(std::ostream &dst, std::string indent) const
{
  dst << indent << "LessThanEqualTo [" << std::endl;
...
// Context
void LessThanEqualToOperator::generateContext(ContextPtr frameContext, int parameterOffset, bool isGlobal  ) {
...
// MIPS
void LessThanEqualToOperator::generateMIPS(std::string indent, int destReg, ContextPtr frameContext) const
{
  switch(type){
    case _float:{
      ...
      frameContext->dealloFloatReg(leftReg);
      std::cout <<"c.le.s" << indent << "$f"<<leftReg<<",$f"<<rightReg<<std::endl;
    ...
    case _double:{
      ...
      frameContext->dealloFloatReg(leftReg);
      std::cout <<"c.le.d" << indent << "$f"<<leftReg<<",$f"<<rightReg<<std::endl;
      std::cout << "bc1f" << indent << "$" << label1 << std::endl;
      std::cout << "nop" << std::endl;
    ...
    default:{
      ...
      std::cout << "slt" << indent << "$" << destReg << ",$" << rightReg << ",$" << leftReg << std::endl; // Find greater than (switch left & right), then flip (can achieve equal to in this way)
...
bool LessThanEqualToOperator::isAssignmentExp() const
{
  return false;
}
void LessThanEqualToOperator::countFunctionParameter(int* functionCallParameterOffset) const
{
...
int LessThanEqualToOperator::getValue() const{
  return (branches[0]->getValue() < branches[1]->getValue());
}

enum Specifier LessThanEqualToOperator::returnType() const{
  return type;
}

void LessThanEqualToOperator::generateMIPSDouble(std::string indent, int destReg, ContextPtr frameContext) const{ 
  std::string number_label = frameContext->getFloatDoubleLabel(1.0, type);
  std::string label1 = makeName("L");
  std::string label2 = makeName("L");
  int leftReg = frameContext->alloFloatCalReg();
