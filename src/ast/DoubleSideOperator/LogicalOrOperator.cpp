...
LogicalOrOperator::LogicalOrOperator(NodePtr Left, NodePtr Right)
{
...
// Visualization
void LogicalOrOperator::generateParser(std::ostream &dst, std::string indent) const
{
  dst << indent << "LogicalOr [" << std::endl;
...
// Context
void LogicalOrOperator::generateContext(ContextPtr frameContext, int parameterOffset, bool isGlobal  ) {
...
// MIPS
void LogicalOrOperator::generateMIPS(std::string indent, int destReg, ContextPtr frameContext ) const
{
  switch(type){
    case _float:{
      std::string label1 = makeName("L");
      std::string label2 = makeName("L");
      std::string label3 = makeName("L");
      ...
      frameContext->alloFloatReg(immReg);

      std::cout << "mtc1" << indent << "$0,$f"<<zeroReg<<std::endl;
      std::cout << "c.eq.s" << indent << "$f" << immReg << ",$f" << zeroReg << std::endl;
      ...
      std::cout << "mtc1" << indent << "$0,$f"<<zeroReg<<std::endl;
      std::cout << "c.eq.s" << indent << "$f" << immReg << ",$f" << zeroReg << std::endl;
      std::cout << "bc1t" << indent << "$" << label2 << std::endl;
      ...
      std::cout << "b" << indent << "$" << label3 << std::endl;
    ...
    case _double:{
      ...
      std::cout << "c.eq.d" << indent << "$f" << immReg << ",$f" << zeroReg << std::endl;
      std::cout << "bc1f" << indent << "$" << label1 << std::endl;
    ...
    default:{
      ...
      std::cout << "bne" << indent << "$" << leftReg << ",$0,$" << label1 << std::endl;
      ...
      frameContext->dealloReg(leftReg);
      std::cout << "beq" << indent << "$" << rightReg << ",$0,$" << label2 << std::endl;
...
bool LogicalOrOperator::isAssignmentExp() const
{
  return false;
}

void LogicalOrOperator::countFunctionParameter(int* functionCallParameterOffset) const
{
...
int LogicalOrOperator::getValue() const{
  return (branches[0]->getValue() || branches[1]->getValue());
}

enum Specifier LogicalOrOperator::returnType() const{
  return type;
}

void LogicalOrOperator::generateMIPSDouble(std::string indent, int destReg, ContextPtr frameContext) const{ 
  std::string label1 = makeName("L");
  std::string label2 = makeName("L");
  std::string label3 = makeName("L");
  int immReg = frameContext->alloFloatCalReg();
