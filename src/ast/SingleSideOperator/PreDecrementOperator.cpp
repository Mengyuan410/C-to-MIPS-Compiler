...
PreDecrementOperator::PreDecrementOperator(NodePtr Left)
{
...
// Visualization
void PreDecrementOperator::generateParser(std::ostream &dst, std::string indent) const
{
  dst << indent << "PreDecrement [" << std::endl;
...
// Context
void PreDecrementOperator::generateContext(ContextPtr frameContext, int parameterOffset, bool isGlobal ) {
...
// MIPS
void PreDecrementOperator::generateMIPS(std::string indent, int destReg, ContextPtr frameContext ) const
{ 
...
void PreDecrementOperator::countFunctionParameter(int* functionCallParameterOffset) const
{
  branches[0]->countFunctionParameter(functionCallParameterOffset);
}

void PreDecrementOperator::generateMIPSDouble(std::string indent, int destReg, ContextPtr frameContext) const{
  int offset = frameContext->getOffset(branches[0]->returnIdentifier());
  std::string number_label = frameContext->getFloatDoubleLabel(1.0, type);
  frameContext->alloFloatReg(destReg);
  int immReg = frameContext->alloFloatCalReg(); 
...
bool PreDecrementOperator::getHasPointerForArithmetic() const{
