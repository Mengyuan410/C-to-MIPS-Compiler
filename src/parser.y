...
declaration_specifiers
	...
	| storage_class_specifier{std::cerr << "Unsupported" << std::endl;}
...
type_specifier
	...
	| enum_specifier { $$ = $1; }
	| TYPE_NAME { $$ = new TypeDefinedName (*$1); delete $1; }
	;
...
direct_declarator
	...
	| '(' declarator ')' { $$ = $2 ;}
	| direct_declarator '(' identifier_list ')' { std::cerr << "Unsupported" << std::endl; }  // Double check
...
expression_statement
	:  expression ';' { $$ = $1; }
	| ';' { }   // TBD but I guess it's already working now (in for loop)
	;
...
expression
	: assignment_expression {$$ = $1; }
	| expression ',' assignment_expression { std::cerr << "Unsupported" << std::endl; }  //Double check
	;
...
unary_expression
	...
	| '+' cast_expression { $$ = new PositiveOperator($2); }
	...
	| SIZEOF '(' type_specifier ')' { $$ = new SizeOfOperator($3); }   
	| SIZEOF '(' unary_expression ')' { $$ = new SizeOfOperator($3); } 
	;
...
struct_or_union_specifier
	...
	| struct_or_union '{' struct_declaration_list '}' { $$ = new StructSpecifier(*$3); delete $3; }  // double check
...
struct_declaration
	: type_specifier struct_declarator ';' { $$ = new StructDeclaration($1,$2);} //assume only int x;
	;

specifier_qualifier_list
	: type_specifier specifier_qualifier_list { std::cerr << "Unsupported" << std::endl; } // TBD
	| type_specifier { std::cerr << "Unsupported" << std::endl; } // TBD
	| type_qualifier specifier_qualifier_list { std::cerr << "Unsupported" << std::endl; } //TBD?
	| type_qualifier { std::cerr << "Unsupported" << std::endl; } // TBD?
	;

struct_declarator_list
	: struct_declarator { std::cerr << "Unsupported" << std::endl; }  // use struct_declarator straight away in struct declaration
...
enum_specifier
	: ENUM '{' enumerator_list '}' { $$ = new EnumSpecifier(*$3); delete $3; } 
...
identifier_list
	: IDENTIFIER { std::cerr << "Unsupported" << std::endl; } // double check
	| identifier_list ',' IDENTIFIER { std::cerr << "Unsupported" << std::endl; }
	;

type_name
	: specifier_qualifier_list { std::cerr << "Unsupported" << std::endl; } //TBD for sizeof
...
labeled_statement
	: IDENTIFIER ':' statement { std::cerr << "Unsupported" << std::endl; }  // double check
